...
mymap.paths <- ggplot() +
  ...
  coord_sf(xlim = c(3300000,4300000),
           ylim = c(3100000,4000000),
           default_crs  = ns_crs) +
  geom_point(data = plotdat, aes(x = x, y = y, colour = agent, group = agent)) +
  geom_path(data = plotdat, aes(x = x, y = y, colour = agent, group = agent)) +
...
path.animate.plot <- mymap.paths +
  transition_reveal(along = day, range = c(min(plotdat$day),max(plotdat$day))) +
  labs(title = 'Date: {frame_along}')  # Add a label on top to say what date each frame is
path.animate.plot.2 <- mymap.paths.2 +
  transition_reveal(along = day, range = c(min(plotdat$day),max(plotdat$day))) +
  labs(title = 'Date: {frame_along}')  # Add a label on top to say what date each frame is
...
We simulate the movement of `n_agents` from March 1 to Nov 30, following the whale densities.\
...
Each of the following plots shows the tracks of 4 agents chosen at random from March 1 to November 30. The label "Date" on top of each plot refers to the day of the year, each animation starts on Date 60 (March 1), and finishes on Date 334 (Nov 30).     

```{r animate, echo = FALSE, message = FALSE}
path.animate.plot
```

```{r animate2, echo = FALSE, message = FALSE}
path.animate.plot.2
```
